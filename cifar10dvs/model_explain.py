...
class MLP(nn.Module):
    ...
    def forward(self, x):
        T,B,C,N = x.shape
        # torch.Size([16, 16, 256, 64])
        x = self.fc1_conv(x.flatten(0,1))
        # torch.Size([256, 1024, 64])
        x = self.fc1_bn(x).reshape(T,B,self.c_hidden,N).contiguous()
        # torch.Size([16, 16, 1024, 64])
        x = self.fc1_lif(x)
        # torch.Size([16, 16, 1024, 64])
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        #和imagenet一样有一个DropPath层，这在cifar10中没有使用
...
class Spikformer(nn.Module):
    def __init__(self,
        ...
        # 这几个pos_embed不知道有什么用，我把和pos_embed相关的都去掉也可以运行
    ...
    def forward(self, x):
        # DVS叫做dynamic vision sensor，代表使用了动态视觉传感器扫描每张 images得到的spike数据
        # DVS产生两个通道的脉冲事件，命名为 On 和Off 事件 (分别如图2中红色和蓝色所示)。因此，DVS 将每个图像转换为T*2*ROW*COLUMN的脉冲模式。
        # 问1：不知道这里为什么是16，2，128，128和原来的图像3，32，32有关吗
        # [N, T, 2, *, *]
        # torch.Size([16, 16, 2, 128, 128])
